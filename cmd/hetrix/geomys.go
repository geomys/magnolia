...
func definitions(h *ht.Hetrix) {
	...
	// TODO: switch to ProdWebsite once ready.
	he.StagingWebsite(ht.Website{
...
func node(h hetrixEnv, prod, staging string) {
	...
	for _, shard := range []string{
		...
		h.ProdWebsite(ht.Website{
			...
			// TODO: use uptime.geomys.org.
			Target:   "https://" + prod + shard + ".sunlight.geomys.org/ct/v1/get-roots",
			Method:   "HEAD",
			Category: "Sunlight",
		})
		h.ProdWebsite(ht.Website{
			Name:     prod + shard + ".skylight.geomys.org",
			Target:   "https://" + prod + shard + ".skylight.geomys.org/checkpoint",
			Keyword:  "— " + prod + shard + ".sunlight.geomys.org",
			Category: "Skylight",
		})
		...
		h.StagingWebsite(ht.Website{
			Name:     staging + shard + ".skylight.geomys.org",
			Target:   "https://" + staging + shard + ".skylight.geomys.org/checkpoint",
			Keyword:  "— " + staging + shard + ".sunlight.geomys.org",
			Category: "Skylight",
		})
	}
}
